	...
	generate 
		for (i = 0; i < SIZE; i++) begin : VECTOR
			Vector #(WIDTH, ADDR) ROWS
					(.address_a		(IB.data_path.input_buffer_addr1[i]),
					.address_b		(IB.data_path.input_buffer_addr2[i]),
					.clock			(IB.clock),
					.data_a			(IB.data_path.input_buffer_data1[i]),
					.data_b			(IB.data_path.input_buffer_data2[i]),
					.rden_a			(IB.control.input_buffer_read),
					.wren_a			(IB.control.input_buffer_write1),
					.wren_b 			(IB.control.input_buffer_write2),
					.q_a				(IB.data_path.input_buffer_output_data[i]));
		end
	endgenerate 
